...
const char FILENAME_FIT_BETASTARS[] = "fitbetastars.txt";
// A csv file with many partial estimates for N = 1000
const char FILENAME_MANY_PARTIAL_ESTIMATES[] = "manypartialestimates.csv";
...
/**
 * Procedure to record K evolutions of the log_estimate of the number
 * of solutions for N = 1000.
 */
void many_log_count_solutions(int M, double beta_del, int K) {
	...
	// Clear partial estimates
	// for (int i = 0; i < 10; i++) {
	// 	fclose(fopen(FILENAME_MANY_PARTIAL_ESTIMATES, "w"));
	// }
	...
	// Get the one for N=1000
	double beta_star = fit_beta_stars[NUM_UNKNOWN-1];
	int T = beta_star / beta_del;
	
	// Record the partial estimates.
	double **partial_estimates = malloc(K * sizeof(double*)); 
	for (int i = 0; i < K; i++) {
		partial_estimates[i] = malloc(T * sizeof(double));
	}	
	...
	for (int i = 0; i < K; i++) {
		...
		for (int t = 1; t <= T; t++) {
			...
			// Otherwise, incorporate the next term.
			log_Z_partial += log_Z_beta_t_ratio;
			partial_estimates[i][t-1] = log_Z_partial + logfact(N);
	...
	// Record approximation.
	for (int i = 0; i < K; i++) {
		save_lf_arr(FILENAME_MANY_PARTIAL_ESTIMATES, partial_estimates[i], T, 1);
	}
	// Free
	free(idx_pairs);
	free(fit_beta_stars);
	for (int i = 0; i < K; i++) {
		free(partial_estimates[i]);
	}
	free(partial_estimates);
...
{	
	...
	// save_lf_arr("logcountestimates.txt", log_count_estimates, NUM_UNKNOWN, 0);
	// free(log_count_estimates);
	
	// UNCOMMENT to get many partial estimates for unknown solutions for N = 1000
	srand(time(NULL));
	many_log_count_solutions(M, beta_del, 1);
